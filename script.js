...
// ========================
// Auto Generate Missing Dates
// ========================
function autoGenerateDates() {
  let savedLastDate = localStorage.getItem("lastDate");
  let todayDate = new Date().toISOString().slice(0, 10);

  // If first time opening app → set lastDate = today
  if (!savedLastDate) {
    localStorage.setItem("lastDate", todayDate);
    return;
  }

  let d1 = new Date(savedLastDate);
  let d2 = new Date(todayDate);

  // If app opened same day → nothing to generate
  if (d1 >= d2) return;

  // Loop from lastDate → today
  while (d1 < d2) {
    d1.setDate(d1.getDate() + 1);
    let nextDate = d1.toISOString().slice(0, 10);

    if (!attendanceData[nextDate]) attendanceData[nextDate] = {};

    // Mark all students absent for missing day
    students.forEach((s) => {
      if (!attendanceData[nextDate][s.id]) {
        attendanceData[nextDate][s.id] = "absent";
      }
    });
  }

  // Save updated last date
  localStorage.setItem("lastDate", todayDate);
  localStorage.setItem("attendance", JSON.stringify(attendanceData));
}
